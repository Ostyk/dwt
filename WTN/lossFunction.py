...
def depthCELoss2(pred, gt, ss, outputChannels=16):
    with tf.name_scope("depth_CE_loss"):
        ...
        crossEntropy = -tf.reduce_sum(((1-gt)*tf.log(tf.maximum(1-predSoftmax, epsilon))
                                       + gt*tf.log(tf.maximum(predSoftmax, epsilon)))*ss*crossEntropyScaling,
                                      reduction_indices=[1])
...
def modelTotalLoss(pred, gt, ss, outputChannels=1):
    lossDepthTotal = depthCELoss2(pred=pred, gt=gt, ss=ss,
                                  outputChannels=outputChannels) / (countTotal(ss) + 1)
